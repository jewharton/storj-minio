...
func testObjectNewMultipartUpload(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	info, err := obj.NewMultipartUpload(context.Background(), bucket, "\\", opts)
	if err != nil {
		t.Fatalf("%s : %s", instanceType, err.Error())
	}

	err = obj.AbortMultipartUpload(context.Background(), bucket, "\\", info.UploadID, opts)
	if err != nil {
		switch err.(type) {
...
func testObjectAbortMultipartUpload(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	abortTestCases := []struct {
		...
		{"--", object, info.UploadID, BucketNotFound{}},
		{"foo", object, info.UploadID, BucketNotFound{}},
		{bucket, object, "foo-foo", InvalidUploadID{}},
		{bucket, "\\", info.UploadID, InvalidUploadID{}},
		{bucket, object, info.UploadID, nil},
	}
...
func testListMultipartUploads(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	// Initiate Multipart Upload on the above created bucket.
	info, err := obj.NewMultipartUpload(context.Background(), bucketNames[0], objectNames[0], opts)
	if err != nil {
		// Failed to create NewMultipartUpload, abort.
		t.Fatalf("%s : %s", instanceType, err.Error())
	}
	...
	for i := 0; i < 3; i++ {
		...
		info, err = obj.NewMultipartUpload(context.Background(), bucketNames[1], objectNames[0], opts)
		if err != nil {
			// Failed to create NewMultipartUpload, abort.
			t.Fatalf("%s : %s", instanceType, err.Error())
		}
	...
	for i := 0; i < 6; i++ {
		info, err = obj.NewMultipartUpload(context.Background(), bucketNames[2], objectNames[i], opts)
		if err != nil {
			// Failed to create NewMultipartUpload, abort.
			t.Fatalf("%s : %s", instanceType, err.Error())
		}
...
func testObjectCompleteMultipartUpload(obj ObjectLayer, instanceType string, t TestErrHandler) {
	...
	// Initiate Multipart Upload on the above created bucket.
	info, err := obj.NewMultipartUpload(context.Background(), bucketNames[0], objectNames[0], ObjectOptions{UserDefined: map[string]string{"X-Amz-Meta-Id": "id"}})
	if err != nil {
		// Failed to create NewMultipartUpload, abort.
		t.Fatalf("%s : %s", instanceType, err)
	}
